...
async function sendMessage() {
    ...
    try {
        if (streaming) {
            ...
            // After full-page re-render, show escalation bar if the last bot message requested it
            const allEscalationData = messagesContainer ? messagesContainer.querySelectorAll('.escalation-data') : [];
            const lastEscalation = allEscalationData.length > 0 ? allEscalationData[allEscalationData.length - 1] : null;
            if (lastEscalation && (lastEscalation.dataset.whatsappUrl || lastEscalation.dataset.emailUrl)) {
                showEscalationBar({
                    whatsAppUrl: lastEscalation.dataset.whatsappUrl,
                    whatsAppButtonText: lastEscalation.dataset.whatsappBtn,
                    whatsAppCtaText: lastEscalation.dataset.whatsappCta,
                    emailUrl: lastEscalation.dataset.emailUrl,
                    emailButtonText: lastEscalation.dataset.emailBtn,
                    emailCtaText: lastEscalation.dataset.emailCta,
                });
            }
        }
        ...
        while (true) {
            ...
            for (const line of lines) {
                ...
                if (event.type === 'token') {
                    ...
                    // Show or hide the escalation bar above the input field
                    if (event.whatsAppUrl || event.emailUrl) {
                        showEscalationBar({
                            whatsAppUrl: event.whatsAppUrl,
                            whatsAppButtonText: event.whatsAppButtonText,
                            whatsAppCtaText: event.whatsAppCtaText,
                            emailUrl: event.emailUrl,
                            emailButtonText: event.emailButtonText,
                            emailCtaText: event.emailCtaText,
                        });
                    }
...
function showEscalationBar(data) {
    const bar = document.getElementById('escalation-bar');
    if (!bar) return;

    const actionsEl = document.getElementById('escalation-bar-actions');
    if (!actionsEl) return;
    ...
    if (data.whatsAppUrl) {
        ...
        a.className = 'escalation-pill escalation-pill--whatsapp';
        a.setAttribute('data-log-escalation', 'whatsapp');
        a.innerHTML = `<svg class="escalation-pill-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        ...
        </svg>${escapeHtml(data.whatsAppButtonText || 'Chat via WhatsApp')}`;
        actionsEl.appendChild(a);
    }

    if (data.emailUrl) {
        const a = document.createElement('a');
        a.href = data.emailUrl;
        a.className = 'escalation-pill escalation-pill--email';
        a.setAttribute('data-log-escalation', 'email');
        a.innerHTML = `<i class="bi bi-envelope escalation-pill-icon"></i>${escapeHtml(data.emailButtonText || 'Stuur een e-mail')}`;
        actionsEl.appendChild(a);
    }

    // Always open expanded when new escalation data arrives
    bar.classList.remove('is-collapsed');
    bar.classList.add('is-visible');
    bar.setAttribute('aria-hidden', 'false');

    const toggle = document.getElementById('escalation-bar-toggle');
    if (toggle) {
        toggle.setAttribute('aria-label', 'Contact opties inklappen');
        const icon = toggle.querySelector('i');
        if (icon) icon.className = 'bi bi-chevron-down';
    }
}

function toggleEscalationBar() {
    const bar = document.getElementById('escalation-bar');
    if (!bar) return;
    const collapsed = bar.classList.toggle('is-collapsed');
    const toggle = document.getElementById('escalation-bar-toggle');
    if (toggle) {
        toggle.setAttribute('aria-label', collapsed ? 'Contact opties uitklappen' : 'Contact opties inklappen');
        const icon = toggle.querySelector('i');
